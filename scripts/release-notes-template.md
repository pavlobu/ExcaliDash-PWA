...
- Run `docker compose -f docker-compose.prod.ssl.yml logs backend --tail=200` after rollout and verify startup/migration status.
...
services:
  backend:
    image: pavlobuidenkov/excalidash-pwa-backend:{{TAG}}
  frontend:
    image: pavlobuidenkov/excalidash-pwa-frontend:{{TAG}}
```
